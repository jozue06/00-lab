...
class EmailSender:
    def __init__(
        ...
        self.body_template = body_template or TEMPLATE_PATH.read_text(encoding="utf-8")
    ...
    def send_batch(self, leads: list[Lead]) -> int:
        ...
        for lead in leads:
            if sent_count >= self.settings.max_emails_per_run:
                logger.info(
                    "Reached MAX_EMAILS_PER_RUN limit (%d)",
            ...
            subject, body = self.render_email(lead)
    ...
    def render_email(self, lead: Lead) -> tuple[str, str]:
        context = {
